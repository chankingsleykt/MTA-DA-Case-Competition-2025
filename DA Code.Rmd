...
PredictedLasVegas <-read_csv("Time Series Analysis/driver_forecasts_2025-11-23.csv")
```
...
points_2025 <- points_by_year %>% 
  ...
  rename(pts = `2025`)
PredictedLasVegas <- PredictedLasVegas %>%
  left_join(DriverPrice, by = "Driver") %>%
  mutate(rate = yhat/Price)
```
...
```
```{r}
max_points_event <- sqldf("SELECT Driver, Price, event_id, points_events FROM points_events ORDER BY points_events DESC")
belgian_qualifiers <- sqldf("SELECT driver_code, total_points_qualify FROM qualifypoints_events WHERE event_id = '2022 Belgian Grand Prix' ORDER BY total_points_qualify DESC")
belgian_fastest_lap <- sqldf("SELECT driver_code FROM fastest_lap_points_events WHERE event_id = '2022 Belgian Grand Prix'")
belgian_grid_change <- sqldf("SELECT grid_position, position FROM RaceResult WHERE event_name = 'Belgian Grand Prix' AND year = 2022 AND driver_code = 'VER'")
```
